...
- Before executing any clocking query or correction, confirm the specific date range and employee(s) in scope. If the response is rejected, immediately ask "Did you mean [DATE/EMPLOYEE]?" and re-confirm before re-running.
- When sending clocking/mail notifications via Telegram or email, always include explicit confirmation in both channels with formatted output (e.g., "Unacknowledged clocks for [DATE]: [LIST]").
